...
void message_handler(aegis::core &bot, aegis::gateway::events::message_create obj) {
  ...
  try {
    ...
    // C++17 version
    // const auto [channel_id, guild_id, message_id, member_id] =
    // obj.msg.get_related_ids();
    ...
    if (found == 0) {
      ...
      newp.title(fmt::format(
          "Hello {}, you must be new here, type 'bm start' to get started.",
          obj.msg.author.username));
      ...
      baermelk.url = ("https://cdnb.artstation.com/p/assets/images/images/018/"
                      "177/381/large/wilson-rojas-degr1.jpg?1558546923");

      aegis::gateway::objects::embed start;
      start.title(
          fmt::format("Welcome to Baermelk {}", obj.msg.author.username));
      start.description("This is a freeroaming game where you can adventure to "
                        "gather resources, fight mobs, and gain experience to "
                        "defeat the main boss, BaerMelk.");
      start.color(31);
      ...
      cmdUse.value("Uses a bandage which heals 10 health or a health kit which "
                   "heals you fully");
      cmdTips.name("Tips");
      cmdTips.value("Armor reduces damage taken by 50%.\n Sword reduces damage "
                    "taken in close quarters by 25%. \n Bow reduces damage "
                    "taken in a long range encounter by 25%.");
      cmdFields.push_back(cmdProfile);
      cmdFields.push_back(cmdShop);
      cmdFields.push_back(cmdAdventure);
      cmdFields.push_back(cmdBuy);
      cmdFields.push_back(cmdSell);
      cmdFields.push_back(cmdUse);
      cmdFields.push_back(cmdTips);
      ...
      aegis::gateway::objects::field sword, bow, arrows, armor, healthKit,
          bandages;
      sword.name("Sword");
      ...
      itemFields.push_back(sword);
      itemFields.push_back(bow);
      itemFields.push_back(arrows);
      itemFields.push_back(armor);
      itemFields.push_back(healthKit);
      itemFields.push_back(bandages);
      ...
      aegis::gateway::objects::field lsword, larmor, lbow, larrows, lhealthkit,
          lbandages;
      lsword.name("Sword: ");
      lsword.value(psword);
      ...
      inventory.push_back(lsword);
      inventory.push_back(lbow);
      inventory.push_back(larrows);
      inventory.push_back(larmor);
      inventory.push_back(lhealthkit);
      inventory.push_back(lbandages);
      ...
      if (content == "bm adventure") {
        ...
        case 0:
          ...
          ehealth.name("Health: ");
          encounter.push_back(esituation);
          encounter.push_back(egold);
          encounter.push_back(ehealth);
      ...
      if (content == "bm baermelk") {
        ...
        if (exp >= 10) {
          ...
          actions.name("Congratulations on making it this far. Now time for "
                       "the real test.");
          actions.value(
              "After some time walking, you see Baermelk in the distance...");
        } else {
          actions.name("You don't have enough experience to fight the one and "
                       "only Baermelk");
          int need = 10 - exp;
    ...
    // Send a message, wait for message to successfully be sent, then react to
    // that message
    if (content == "~Delay") {
